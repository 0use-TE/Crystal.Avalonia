...
| Method | What it does |
|--------|--------------|
| `AddMvvmTransient<TView, TViewModel>()` | `AddTransient<TViewModel>()` + View ↔ ViewModel mapping |
| `AddMvvmSingleton<TView, TViewModel>()` | `AddSingleton<TViewModel>()` + View ↔ ViewModel mapping |

`TView` is not registered in DI. Views are created by XAML (View-first) or `Activator.CreateInstance` (ViewLocator).
...
> **How it works:** [Architecture — Bootstrap Pipeline](../architecture.md#bootstrap-pipeline) — when App vs module services register. [Design Decisions](../architecture.md#design-decisions) — ViewModel in DI, View via mapping only.

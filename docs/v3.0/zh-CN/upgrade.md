...
- **`ConfigureOptions` / `CrystalApplication.Options`** → ViewModel-first 的 `DataTemplates`（`EnableViewLocator`）及其他框架开关；同时是 DI 单例
